...
import { getOrganisationData } from "../helpers/lists-helper.js";
...
export async function getUserProfile(callback) {
  try {
    ...
    //localStorage.setItem("office-token", middletierToken);
    let response = await getOrganisationData(middletierToken);
    callback(response);
    if (!response) {
      ...
      //localStorage.setItem("office-token", mfaMiddletierToken);
      let response = await getOrganisationData(mfaMiddletierToken);
