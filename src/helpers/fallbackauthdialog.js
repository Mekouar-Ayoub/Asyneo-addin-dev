...
import { getOrganisationData } from "./lists-helper";

const clientId = "9e771e21-8974-435d-aa55-a7c6a69f8137"; //This is your client ID
...
const msalConfig = {
  auth: {
    clientId: clientId,
    authority: "https://login.microsoftonline.com/" + "f0d36d99-6efa-433a-9920-246dead5de54",
...
export async function dialogFallback(callback) {
  ...
  if (homeAccountId !== null) {
    ...
    if (result !== null && result.accessToken !== null) {
      const response = await getOrganisationData(result.accessToken);
...
async function processMessage(arg) {
  ...
  if (messageFromDialog.status === "success") {
    ...
    const response = await getOrganisationData(messageFromDialog.result);
    callbackFunction(response);
